//Powerful script by portal2001
(function () {
    if (app.documents.length === 0) {
        return alert("There are no open documents to process. Please open some images first.");
    }
    if (parseInt(app.version.split(".")[0]) < 23 && getSetting("outputFormat", "WebP") === "WebP") {
        return alert("You must use Photoshop 2022 (v23) or later to save using the native WebP format.");
    }
    showDialog();
})();
function saveSetting(key, value) {
    ...
    switch (typeof value) {
        case "boolean": desc.putBoolean(stringIDToTypeID(key), value); break;
        case "string": desc.putString(stringIDToTypeID(key), value); break;
        case "number": desc.putDouble(stringIDToTypeID(key), value); break;
        default: break;
    }
...
function getSetting(key, defaultValue) {
    try {
        ...
        switch (type) {
            case DescValueType.BOOLEANTYPE: return desc.getBoolean(stringIDToTypeID(key));
            case DescValueType.STRINGTYPE: return desc.getString(stringIDToTypeID(key));
            case DescValueType.DOUBLETYPE: return desc.getDouble(stringIDToTypeID(key));
...
function showDialog() {
    var win = new Window("dialog", "Processing Portal (v3.0)");
    win.orientation = "column";
    win.alignChildren = "fill";
    win.spacing = 10;
    win.margins = 15;
    var infoText = win.add("statictext", undefined, "WebP files will be saved into a 'webp' subfolder next to each original file.", {multiline: true});
    ...
    var formatDropdown = formatGroup.add("dropdownlist", undefined, ["WebP", "JPEG", "PNG", "TIFF"]);
    ...
    var qualityText = webp_jpeg_Group.add("edittext", undefined, "75", {characters: 4});
    qualitySlider.onChanging = function () { qualityText.text = Math.round(this.value); };
    qualityText.onChange = function () {
        var val = parseInt(this.text);
        if (!isNaN(val) && val >= 0 && val <= 100) { qualitySlider.value = val; } else { this.text = Math.round(qualitySlider.value); }
    };
    ...
    var pngCompressionDropdown = pngGroup.add("dropdownlist", undefined, ["None / Fast", "Smallest File Size"]);
    ...
    var tiffCompressionDropdown = tiffGroup.add("dropdownlist", undefined, ["NONE", "LZW", "ZIP"]);
    ...
    var fitImageCheckbox = fitImageGroup.add("checkbox", undefined, "Fit Longest Edge:");
    var fitImageInput = fitImageGroup.add("editnumber", undefined, 1920, {characters: 5});
    ...
    leftColumn.add("statictext", undefined, "Color:").graphics.font = ScriptUI.newFont("dialog", "BOLD", 13);
    var rgbConversionDropdown = leftColumn.add("dropdownlist", undefined, ["Convert to sRGB space", "Keep Current RGB space"]);
    ...
    var xmpDataCheckbox = rightColumn.add("checkbox", undefined, "Include XMP Data");
    var exifDataCheckbox = rightColumn.add("checkbox", undefined, "Include EXIF Data");
    ...
    var actionSetDropdown = actionSetGroup.add('dropdownlist', undefined, undefined, { preferredSize: [350, 25] });
    ...
    var actionDropdown = actionGroup.add('dropdownlist', undefined, undefined, { preferredSize: [350, 25] });
    var bottomGroup = win.add("group", undefined, {orientation: "row", alignChildren: ["middle", "fill"]});
    var keepOpenCheckbox = bottomGroup.add("checkbox", undefined, "Keep Originals Open");
    var hideProgressCheckbox = bottomGroup.add("checkbox", undefined, "Hide Progress for Max Speed");
    ...
    function updateFormatOptions() {
        ...
        infoText.text = "Files will be saved into a '" + selectedFormat.toLowerCase() + "' subfolder next to each original file.";
    ...
    if (allActionSets && allActionSets.length > 0) { for (var i = 0; i < allActionSets.length; i++) { actionSetDropdown.add('item', allActionSets[i]); } }
    ...
    actionSetDropdown.onChange = function () {
        ...
        if (this.selection.index > 0) {
            ...
            if (allActions && allActions.length > 0) { for (var i = 0; i < allActions.length; i++) { actionDropdown.add('item', allActions[i]); } }
        }
    };
    function loadSettings() {
        formatDropdown.selection = formatDropdown.find(getSetting("outputFormat", "WebP")) || 0;
        ...
        rgbConversionDropdown.selection = getSetting("rgbConversion", 0);
    ...
    fitImageCheckbox.onClick = function () { fitImageInput.enabled = this.value; };
    ppiCheckbox.onClick = function () { ppiInput.enabled = this.value; };
    okButton.onClick = function () {
        ...
        saveSetting("rgbConversion", rgbConversionDropdown.selection.index);
        ...
        var settings = {
            ...
            rgbConversion: rgbConversionDropdown.selection.text,
            fitValue: fitImageCheckbox.value ? parseInt(fitImageInput.text) : null,
            ppi: ppiCheckbox.value ? parseInt(ppiInput.text) : null,
            actionSet: actionSetDropdown.selection.index > 0 ? actionSetDropdown.selection.text : null,
            actionName: actionDropdown.selection.index > 0 ? actionDropdown.selection.text : null,
            hideProgress: hideProgressCheckbox.value,
            keepOpen: keepOpenCheckbox.value
        };
        win.close();
        processOpenDocuments(settings);
    };
    loadSettings();
    win.show();
}
function processOpenDocuments(settings) {
    ...
    if (!settings.hideProgress) {
        progressWin = new Window("palette", "Processing...", undefined, { closeButton: false });
    ...
    for (var i = totalDocs - 1; i >= 0; i--) {
        ...
        var outputFolder = new Folder(sourcePath + "/" + settings.outputFormat.toLowerCase());
        if (!outputFolder.exists) { outputFolder.create(); }
        doc.suspendHistory("Process with Portal", "runConversion(settings, outputFolder)");
    ...
    if (!settings.hideProgress) { progressWin.close(); }
...
function runConversion(settings, outputFolder) {
    ...
    if (settings.rgbConversion === "Convert to sRGB space") {
    ...
    if (settings.actionSet && settings.actionName) { runAction(settings.actionSet, settings.actionName); }
    if (settings.fitValue !== null || settings.ppi !== null) { fitImage(settings.fitValue, settings.ppi); }
    ...
    switch (settings.outputFormat) {
        ...
        case "JPEG":
            ...
            activeDocument.saveAs(new File(outputFolder + "/" + docName + ".jpg"), saveOptions, true);
            break;
        case "PNG":
            saveOptions = new PNGSaveOptions();
            saveOptions.compression = settings.pngCompression === 0 ? 0 : 9;
            saveOptions.interlaced = false;
            activeDocument.saveAs(new File(outputFolder + "/" + docName + ".png"), saveOptions, true);
            break;
        case "TIFF":
            saveOptions = new TiffSaveOptions();
            if (settings.tiffCompression === "LZW") saveOptions.imageCompression = TIFFEncoding.TIFFLZW;
            else if (settings.tiffCompression === "ZIP") saveOptions.imageCompression = TIFFEncoding.TIFFZIP;
            else saveOptions.imageCompression = TIFFEncoding.NONE;
            saveOptions.layers = false;
            activeDocument.saveAs(new File(outputFolder + "/" + docName + ".tif"), saveOptions, true);
            break;
    }
}
function saveWebP(s, outputFolder, docName) {
    ...
    saveOpts.putEnumerated(stringIDToTypeID("compression"), stringIDToTypeID("WebPCompression"), stringIDToTypeID("compressionLossy"));
    ...
    desc.putObject(stringIDToTypeID("as"), stringIDToTypeID("WebPFormat"), saveOpts);
...
function fitImage(fitValue, ppi) { var doc = activeDocument; var resampleMethod = ResampleMethod.BICUBIC; if (fitValue !== null && ppi !== null) { if (doc.height.value > doc.width.value) doc.resizeImage(null, UnitValue(fitValue, "px"), ppi, resampleMethod); else doc.resizeImage(UnitValue(fitValue, "px"), null, ppi, resampleMethod); } else if (fitValue !== null) { if (doc.height.value > doc.width.value) doc.resizeImage(null, UnitValue(fitValue, "px"), doc.resolution, resampleMethod); else doc.resizeImage(UnitValue(fitValue, "px"), null, doc.resolution, resampleMethod); } else if (ppi !== null) { doc.resizeImage(undefined, undefined, ppi, ResampleMethod.NONE); } }
function getActionSets() { var sets = []; try { var i = 1; while (true) { var ref = new ActionReference(); ref.putIndex(charIDToTypeID('ASet'), i); var desc = executeActionGet(ref); sets.push(desc.getString(charIDToTypeID('Nm  '))); i++; } } catch(e) {} return sets; }
function getActions(setName) { var actions = []; try { var i = 1; while (true) { var ref = new ActionReference(); ref.putIndex(charIDToTypeID('Actn'), i); ref.putName(charIDToTypeID('ASet'), setName); var desc = executeActionGet(ref); actions.push(desc.getString(charIDToTypeID('Nm  '))); i++; } } catch(e) {} return actions; }
function runAction(actionSet, actionName) { try { app.doAction(actionName, actionSet); } catch (e) {} }